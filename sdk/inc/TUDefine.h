...
typedef enum
{
    ...
    TUIDI_CAMERA_PAYLOADSIZE,                      // the Camera PayloadSize
    TUIDI_CAMERA_LOG,                              // the Camera Log Message
...
typedef enum
{
	...
	TUIDC_ATWBALANCE_TYPE		= 0x49,             // id capability automatic white balance type(0-Normal 1-AI mode)
    TUIDC_ENDCAPABILITY         = 0x4A,             // id capability end 
}TUCAM_IDCAPA;
...
{   
	...
	TUIDV_LOW_GAIN_CORRECT = 0x1C,             // id vendor the low gain correct
    TUIDV_ENDVPROPERTY = 0x1D,             // id vendor end 
}TUCAM_IDVPROP;
...
typedef enum
{
	...
	TU_CAMERALINKITF_XML = 0x02,     // [in] the device of the camera interface link xml
    TU_CAMERABASE_XML = 0x03,        // [in] the host device of the camera xml
} TUXML_DEVICE;
...
//  typedef struct chunk data buf info
typedef struct _tagTUCAM_CHUNK_DATA
{
	UINT32 uiID;			// [out] The image chunk data id
	UINT32 uiSize;			// [out] The image chunk data size
	
	PUCHAR pChunkData;      // [out] Pointer to the chunk data
} TUCAM_CHUNK_DATA, *PTUCAM_CHUNK_DATA;
...
typedef struct _tagTUCAM_CB_BUFHEADER
{
	...
	UINT32 uiBufHandle;     // [out] The buf handle
...
typedef enum
{
    TUDATA_READ = 0x00,	// [in] data readout
    TUDATA_WRITE = 0x01	// [in] data write 
}TUDATA_RW; 

typedef enum
{
	TU_CAMERA_TYPE_CONFIG,
	TU_VERSION_TYPE_CONFIG,
	TU_GENICAM_ISDONE_CONFIG
}TU_API_CONFIG_TYPE;
...
//  typedef TDI struct call back buf info
typedef struct _tagTUCAM_TDI_CB_BUFHEADER
{
    TUCAM_CB_BUFHEADER cb_header;
    UINT32 uiImageID;       // [out] The total number of this open flow
} TUCAM_TDI_CB_BUFHEADER, *PTUCAM_TDI_CB_BUFHEADER;
#endif  // _TUDEFINE_H_